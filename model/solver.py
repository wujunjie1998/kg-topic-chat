...
from utils import to_var, time_desc_decorator, TensorboardWriter, pad_and_pack, normal_kl_div, to_bow, bag_of_words_loss, normal_kl_div, embedding_metric,pad
import metrics
import os
...
class Solver(object):
    def __init__(self, config, train_data_loader, eval_freq_data_loader, eval_rare_data_loader, vocab, is_train=True, model=None):
        self.config = config
        self.epoch_i = 0
        self.train_data_loader = train_data_loader
        self.eval_freq_data_loader = eval_freq_data_loader
        self.eval_rare_data_loader = eval_rare_data_loader
        self.test_data_loader = eval_freq_data_loader
        self.vocab = vocab
    ...
    def write_summary(self, epoch_i):
        ...
        validation_freq_loss = getattr(self, 'validation_freq_loss', None)
        if validation_freq_loss is not None:
            self.writer.update_loss(
                loss=validation_freq_loss,
                step_i=epoch_i + 1,
                name='validation_freq_loss')

        validation_rare_loss = getattr(self, 'validation_rare_loss', None)
        if validation_rare_loss is not None:
            self.writer.update_loss(
                loss=validation_rare_loss,
                step_i=epoch_i + 1,
                name='validation_rare_loss')
    ...
    def train(self):
        ...
        for epoch_i in range(self.epoch_i, self.config.n_epoch):
            ...
            for batch_i, (conversations, conversation_length, sentence_length) in enumerate(tqdm(self.train_data_loader, ncols=80)):
                ...
                conv_indices = [i for i in range(len(conversations))]
                sentence_length = [c for i in conv_indices for c in [sentence_length[i][:-1]]]
            ...
            #print('\n<Validation>...')
            self.validation_freq_loss, self.validation_rare_loss = self.evaluate()
    ...
    def evaluate(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) in enumerate(tqdm(self.eval_freq_data_loader, ncols=80)):
            # conversations: (batch_size) list of conversations
            #   conversation: list of sentences
            #   sentence: list of tokens
            # conversation_length: list of int
        ...
        for batch_i, (conversations, conversation_length, sentence_length) in enumerate(tqdm(self.eval_rare_data_loader, ncols=80)):
            # conversations: (batch_size) list of conversations
            #   conversation: list of sentences
            #   sentence: list of tokens
            # conversation_length: list of int
    ...
    def test(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) in enumerate(tqdm(self.test_data_loader, ncols=80)):
            ...
            generated_sentences = self.generate_conversations_with_gold_responses(input_sentences,
                                       input_sentence_length,
                                       input_conversation_length,
                                       target_sentences)
            conv_f1 = 0
            for target_sent, output_sent in zip(target_sentences, generated_sentences):
    ...
    def generate_conversations(self, input_sentences, input_sentence_length,
                          input_conversation_length, target_sentences):
        self.model.eval()
    ...
    def generate_conversations_with_gold_responses(self, input_sentences, input_sentence_length,
                          input_conversation_length, target_sentences):
        self.model.eval()
    ...
    def generate_conversations_with_beam_decode(self, input_sentences, input_sentence_length,
                          input_conversation_length, target_sentences):
        self.model.eval()
    ...
    def embedding_metric(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) \
                in enumerate(tqdm(self.eval_freq_data_loader, ncols=80)):
            # conversations: (batch_size) list of conversations
            #   conversation: list of sentences
            #   sentence: list of tokens
            # conversation_length: list of int
